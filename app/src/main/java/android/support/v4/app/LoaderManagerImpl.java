...
class LoaderManagerImpl extends LoaderManager {
    ...
    public <D> Loader<D> initLoader(int r5, Bundle bundle, LoaderManager.LoaderCallbacks<D> loaderCallbacks) {
        if (this.mCreatingLoader) {
            throw new IllegalStateException("Called while creating a loader");
        }
        LoaderInfo loaderInfo = this.mLoaders.get(r5);
        if (DEBUG) {
            Log.v(TAG, "initLoader in " + this + ": args=" + bundle);
        }
        if (loaderInfo == null) {
            loaderInfo = createAndInstallLoader(r5, bundle, (LoaderCallbacks<Object>) loaderCallbacks);
            if (DEBUG) {
            ...
            loaderInfo.mCallbacks = (LoaderCallbacks<Object>) loaderCallbacks;
        }
    ...
    @Override // android.support.v4.app.LoaderManager
    public <D> Loader<D> restartLoader(int r7, Bundle bundle, LoaderManager.LoaderCallbacks<D> loaderCallbacks) {
        if (this.mCreatingLoader) {
            throw new IllegalStateException("Called while creating a loader");
        }
        LoaderInfo loaderInfo = this.mLoaders.get(r7);
        if (DEBUG) {
            Log.v(TAG, "restartLoader in " + this + ": args=" + bundle);
        }
        if (loaderInfo != null) {
            LoaderInfo loaderInfo2 = this.mInactiveLoaders.get(r7);
            if (loaderInfo2 != null) {
                if (loaderInfo.mHaveData) {
                    ...
                    this.mInactiveLoaders.put(r7, loaderInfo);
                } else if (!loaderInfo.mStarted) {
                    if (DEBUG) {
                        Log.v(TAG, "  Current loader is stopped; replacing");
                    }
                    this.mLoaders.put(r7, null);
                    loaderInfo.destroy();
                } else {
                    ...
                    loaderInfo.mPendingLoader = createLoader(r7, bundle, (LoaderCallbacks<Object>) loaderCallbacks);
        ...
        return (Loader<D>) createAndInstallLoader(r7, bundle, (LoaderCallbacks<Object>) loaderCallbacks).mLoader;
    }
    ...
    @Override // android.support.v4.app.LoaderManager
    public <D> Loader<D> getLoader(int r4) {
        if (this.mCreatingLoader) {
            throw new IllegalStateException("Called while creating a loader");
        }
        LoaderInfo loaderInfo = this.mLoaders.get(r4);
        if (loaderInfo != null) {
            if (loaderInfo.mPendingLoader != null) {
                return (Loader<D>) loaderInfo.mPendingLoader.mLoader;
            }
